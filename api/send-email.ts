 ...
 *
 * Receives a planner submission and sends a formatted HTML email
 * (with a Markdown attachment) to Raven Digital via Gmail SMTP.
 *
 * Requires two environment variables:
 *   GMAIL_USER          — Gmail address used to send
 *   GMAIL_APP_PASSWORD  — 16-char Gmail App Password (not the account password)
 *
 * Returns 503 when credentials are missing (graceful degradation — the
 * frontend will show an amber notice; WhatsApp remains the primary channel).
 */
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    console.error("Failed to send email", { err, refNumber });
